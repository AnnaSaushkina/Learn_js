...
const descendingOrder = numbers => {
    return +numbers.toString().split('').sort((a,b) => b - a).join('')
};
...
const getThirdMaxNum = (numbers) => {
    ...
    return uniqNumbers.length >= 3 ? uniqNumbers[2] : uniqNumbers[uniqNumbers.length - 1]
}
...
const textToBinary = (str) => {
    const testStr = str.toLowerCase().split(' ').map((word) => {
...
console.log(mostFrequent(['apple', 'banana', 'apple', 'orange', 'banana', 'banana'])); // 'banana'
